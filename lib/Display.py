...
class TickerThread(Thread):
    ...
    def run(self):
        Tools.debug("StickerThread started")
        ...
        Tools.debug("StickerThread terminated")
        self._isAlive = False
    ...
    def __init__(self):
        ...
        try:
            if GPIO.RPI_REVISION > 1:
                self._bus = smbus.SMBus(1) # For revision 1 Raspberry Pi, change to bus = smbus.SMBus(1) for revision 2.
            else:
                self._bus = smbus.SMBus(0) # For revision 1 Raspberry Pi, change to bus = smbus.SMBus(1) for revision 2.
            self._bus.write_byte_data(_addr, 0x00, 0x00) # Set all of bank 0 to outputs
            self._bus.write_byte_data(_addr, 0x01, 0x00) # Set all of bank 1 to outputs
            self._bus.write_byte_data(_addr, 0x13, 0xff) # Set all of bank 1 to high (all digits off)
            Tools.debug("Display instance created")
        except:
